...
## 具体操作说明
**注意事项：**
* 使用前请务必注意：牛牛目前只是个测试版本，没有任何安全鉴权，只可用在可信任的局域网内，切记！
* 如果需要错误信息，则下单程序要打开弹框，如果不需要可以关掉弹框，下单买卖速度更快

### 运行牛牛
牛牛是同花顺下单程序和ThsTrader之间的代理Proxy，负责转发指令，以及提供webservice等相关REST服务, 目前免费测试中。
1. 运行`assets/niuniu`目录下的`niuniu.exe`, 选择同花顺下单程序`xiadan.exe`, 点击`执行`，拉起下单程序后，手工输入账号密码登录。 (必须按此步骤，不可变更)
2. 记录牛牛页面的`标识`，后续程序需要
...
### 实例化Trader
```python
ip = '192.168.0.100' #以机器实际IP为准, 本机可以是localhost或127.0.0.1
mark = 3721 # 牛牛的标识列
...
## 支持券商
* 理论上只要券商支持同花顺下单都可以支持
* 下单主程序`xiadan.exe`要求版本`5.18`版, 过高或者过低都没测，不知道是否好用

## 技术原理
* 和其他几个开源的键盘鼠标模拟(例如`pywinauto`)类似，都是从底部驱动键盘和鼠标操作
* 需要单独下载辅助程序`niuniu.exe`

## 常见问题
* 为什么无法连接？
  - 首先检查`xiadan.exe`的版本，必须是5.18版(非THS通用版)，最好是券商网站下载的`同花顺独立下单程序`
  - 执行顺序必须按照`运行牛牛`的顺序进行，不可改动
